<template>
  <form class="mb-3" @submit.prevent="editBug()">
...
export default {
  ...
  setup() {
    ...
    return {
      ...
      async editBug() {
        try {
          const bug = await bugsService.editBug(AppState.activeBug.id, editable.value)
          Pop.toast('Bug Editied', 'success')
          const modal = Modal.getInstance(document.getElementById('bug-modal-edit'))
          modal.hide()
        } catch (error) {
          logger.log(error)
        }
